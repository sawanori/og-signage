 ...
 *   /dev/dashboard?role=staff|administrator&state=default|empty|no-videos|no-preview&preview=portrait|landscape
 * 既定はモック（image/dashboard.png）と同じ内容。視覚比較は tests/visual/dashboard.spec.ts。
 * プレビューの向きは preview、なければ /admin と同じ cookie、どちらも無ければ縦。
 */
...
export default async function DevDashboardPage({
  ...
  const orientation =
    parsePreviewOrientation(typeof params.preview === "string" ? params.preview : null) ??
    parsePreviewOrientation((await cookies()).get(PREVIEW_ORIENTATION_COOKIE)?.value) ??
    "portrait";
  const previewConfig = state === "empty" || state === "no-preview" ? null : mockConfig(orientation);
