...
1. **表示スケジュール**: 曜日ごとの表示時間帯。時間外は HDMI 出力を切り、動画も止める。出力を切れない場合は黒画面。日をまたぐ時間帯（22:00〜翌6:00 など）を許す。未設定の曜日は終日表示。〔要確認：ユーザー未確定の仮仕様〕
2. **権限表**:
...
3. **参加人数**: スタッフが手入力する。
4. **天気 API**: task_002 で選定する（商用利用可否・キー要否・呼び出し上限を記録）。〔要確認：契約〕
...
15. **音声**: 動画の音量は端末ごとの設定で、初期値は 0（ミュート）。〔要確認〕
...
- **今日の主イベント**: 開催中のものを優先し、なければ今日これから始まるもの。同じ条件なら開始が早い順、次に作成が早い順。今日のイベントがなければ「次のイベント」を小さく出し、それもなければハウスのキャッチコピーを出す。
- **Upcoming**: 主イベントを除く、明日以降（今日のうち主イベントより後のものを含む）の公開イベントを開始順に最大 4 件（縦型・横型ともモックの枠数。要件の「最大 5 件程度」の範囲内）。
- **今週の予定**: 月曜始まりの 7 日間。
