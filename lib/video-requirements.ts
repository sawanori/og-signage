 ...
 * 20 秒以内・12MB 以下。
 */
import { MAX_VIDEO_SECONDS, MEDIA_MAX_BYTES, isVideoTooLong } from "./file-sniff";
...
export function videoRequirementViolations(facts: VideoFacts): string[] {
  ...
  if (facts.fileSize > MEDIA_MAX_BYTES.video) {
    problems.push(
      `ファイルの大きさ: ${(facts.fileSize / 1_000_000).toFixed(1)}MB（12MB 以下にしてください。20 秒の 1920×1080 なら書き出しのビットレートを 4Mbps 程度に）`,
    );
  }
