...
describe("プレイリスト: 追加", () => {
  ...
  it("15 秒を超える動画は再生リストに入れられない", async () => {
    await addPlaylist("pl1");
    const long = await addMedia({ durationSeconds: 30 });
    const error = await expectServiceError(addPlaylistItem(db, "pl1", { revision: 0, mediaId: long }), "invalid_media", 400);
    expect(error.message).toBe("この動画は 15 秒を超えているため再生リストに入れられません");
    const fifteen = await addMedia({ durationSeconds: 15 });
    await expect(addPlaylistItem(db, "pl1", { revision: 0, mediaId: fifteen })).resolves.toBeTruthy();
  });
